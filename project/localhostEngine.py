...
try:
	file = open('client_secrets_localHost.json','r')
	words = file.read()
	pythonObj = json.loads(words)
	LOCAL_REDIRECT_URI = str(pythonObj['web']['redirect_uris'][0])
	LOCAL_CLIENT_ID = str(pythonObj['web']['client_id'])
	LOCAL_CLIENT_SECRET =  str(pythonObj['web']['client_secret'])
	LOCAL_HOME = str(pythonObj['web']['javascript_origins'][0])
	# print LOCAL_REDIRECT_URI
	# print LOCAL_CLIENT_ID
	# print LOCAL_CLIENT_SECRET
...
def signIn():
	# global signInFlag
	# signInFlag = True
	flow = flow_from_clientsecrets('client_secrets_localHost.json', scope = LOCAL_SCOPE, redirect_uri = LOCAL_REDIRECT_URI)
...
def index():
	...
	if 'email' in s:
		...
		return template('views/localHostHomePage.tpl', keyTimesTuple =  keyTimesTuple, keywords = keywords, keyListInInputOrder = keyListInInputOrder, keyDic = keyDic, user_email = email,picture = picture, name = name, searchword = allUserSearchWord[email])

	# not logged in 
	else:
		print "i am not logged in..."
		return template('views/localHostHomePage.tpl', keyTimesTuple =  None, keywords = keywords, keyListInInputOrder = keyListInInputOrder, keyDic = keyDic, user_email = None, picture = None, name = None, searchword = None)
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
@route('/redirect') 
def redirect_page():

	code = request.query.get('code', '')
	flow = OAuth2WebServerFlow( client_id= LOCAL_CLIENT_ID, client_secret= LOCAL_CLIENT_SECRET,scope=LOCAL_SCOPE, redirect_uri=LOCAL_REDIRECT_URI)
...
def logout():
	...
	redirect('https://www.google.com/accounts/Logout?continue=https://appengine.google.com/_ah/logout?continue=' + LOCAL_HOME)
	#redirect('/')
...
def searchResultPage(pageid):
 ...
 if int(pageid) == 0:
	...
	if 'email' in s:
		return template('views/localHostSearchResults.tpl', picture = picture, database_urlToWords = database_urlToWords, sortedUserURL = sortedUserURL[currPageIdLeft:currPageIdRight], keywords = keywords, pageNum = pageNum, pageid = int(pageid), docTitle = database_urlToTitle, searchword = allUserSearchWord[email])
	else:
		return template('views/localHostSearchResults.tpl', picture = picture, database_urlToWords = database_urlToWords, sortedUserURL = sortedUserURL[currPageIdLeft:currPageIdRight], keywords = keywords, pageNum = pageNum, pageid = int(pageid), docTitle = database_urlToTitle, searchword = None)
